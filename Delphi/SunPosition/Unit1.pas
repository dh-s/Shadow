...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure cbDltSavClick(Sender: TObject);
...
procedure TForm1.cbDltSavClick(Sender: TObject);
var
  ndx, i : integer;
begin
  ndx := strtoint ((TDateTimePicker(Sender)).Name[9]);
...
BEGIN
  if s = '' then begin
    Form1.lblMsg.Caption := 'Time Zone must be selected';
...
begin
  if s.Length = 0 then begin
    if allowblank then exit
      ...
      Form1.lblMsg.Caption := 'Longitude must be entered';
      Form1.lblMsg.Font.Color := clRed;
   ...
   if CheckDegFormat (s) then begin
    ...
    if (l > 180) or (l < -180) then begin
      Form1.lblMsg.Caption := 'Max longitude allowed is 180';
      Form1.lblMsg.Font.Color := clRed;
      Form1.edLongitude.color := clRed;
      sinceErr := 0;
    end
    ...
    Form1.lblMsg.Caption := 'Invalid format for longitude';
    Form1.lblMsg.Font.Color := clRed;
    Form1.edLongitude.color := clRed;
    sinceErr := 0;
  end;
...
begin
  if s.Length = 0 then begin
    if allowblank then exit
      ...
      Form1.lblMsg.Caption := 'Latitude must be entered';
      Form1.lblMsg.Font.Color := clRed;
  ...
  if CheckDegFormat (s) then begin
    ...
    if (l > 89) or (l < -89) then begin
      Form1.lblMsg.Caption := 'Max latitude allowed is 89';
      Form1.lblMsg.Font.Color := clRed;
      Form1.edLatitude.color := clRed;
      sinceErr := 0;
    end
    ...
    Form1.lblMsg.Caption := 'Invalid format for latitude';
    Form1.lblMsg.Font.Color := clRed;
    Form1.edLatitude.color := clRed;
    sinceErr := 0;
  end;
      ...
      if Dates[i].edDate.Checked then begin
        if Times[j].Checked then begin
          ...
          if Results[i, j].Altitude < 1 then begin
            Results[i, j].lblResult.Caption := 'sun too low';
            Results[i, j].lblResult.Cursor := crDefault;
            Results[i, j].Altitude := 0;
          end
          else begin
            Results[i, j].lblResult.Caption :=
                'Al: ' + DegtoStr(Results[i, j].Altitude) + SLineBreak +
                'Az: ' + DegtoStr(Results[i, j].Azimuth);
...
begin
  ...
  for i:= 1 to 4 do
    ...
    edLatitude.Text := Ini.ReadString ('Calculator', 'Latitude', '41.88421');
    edLongitude.Text := Ini.ReadString ('Calculator', 'Longitude', '-72.801035');
    edZone.ItemIndex := Ini.ReadInteger ('Calculator', 'ZoneNdx', 31);
    rbDecDeg.Checked := Ini.ReadBool('Calculator', 'DecimalDegrees', true);
    rbDegMinSec.Checked := not rbDecDeg.Checked;
    edPlace.Text := Ini.ReadString ('Calculator', 'Place', 'Simsbury CT');
    edAbrev.Text := Ini.ReadString ('Calculator', 'Place Abrev', 'Simsbury CT');
	finally
		Ini.Free;
	end;
  for i := 1 to 4 do
      ...
      if Times[j].Checked and (Results[i,j].Altitude > 0) then begin
        ...
        if not DirectoryExists (filename) then
          if not CreateDir (filename) then begin
            Form1.lblMsg.Caption := 'Error creating Shad directory';
            Form1.lblMsg.Font.Color := clRed;
          end;
...
begin
  if length(Trim (edAbrev.Text)) = 0 then begin
    lblMsg.Caption := 'Must Enter Place Abbreviation';
    ...
    try
      ...
      lblMsg.Caption := 'Added to Saved Sun Positions && filter set';
    ...
    lblMsg.Caption := 'Nothing to Export (select Calculate button first)';
    lblMsg.Font.Color := clRed;
  end;
...
begin
  ...
  timeNdx := strtoint(s[s.Length]);
 	SaveResultToIni (dateNdx, timeNdx);
  lblMsg.Font.Color := clBlue;
  lblMsg.Caption := 'Altitude && Azimuth transferred to DataCAD';
end;
